...
<body class="bg-gradient-to-b from-gray-50 to-white text-gray-800 antialiased">
    ...
    <header class="bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-500 text-white">
        ...
        @if(isset($openaiStatus['status']) && $openaiStatus['status'] !== 'ok')
            <div class="w-full text-sm text-center py-2 px-4" style="background: rgba(0,0,0,0.08);">
                ...
                @elseif($openaiStatus['status'] === 'quota')
                    <div class="max-w-7xl mx-auto text-yellow-100">OpenAI: quota exceeded — using local fallback. (<span class="font-semibold">{{ $openaiStatus['message'] }}</span>)</div>
                @else
                    <div class="max-w-7xl mx-auto text-yellow-100">OpenAI: unavailable — using local fallback. (<span class="font-semibold">{{ $openaiStatus['message'] }}</span>)</div>
                @endif
            </div>
        @endif
        <div class="max-w-7xl mx-auto px-6 py-20 lg:py-28 flex flex-col lg:flex-row items-center gap-12">
            <div class="lg:w-1/2 text-center lg:text-left">
                <div class="flex items-center justify-center lg:justify-start gap-3 mb-4">
                    <img src="{{ asset('logo.svg') }}" alt="Logo" class="w-10 h-10 rounded" />
                    <div class="text-left">
    ...
    <footer class="bg-white border-t mt-12">
        <div class="max-w-7xl mx-auto px-6 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="text-sm text-gray-600">© {{ date('Y') }} Dilmak Solutions — AI Property Description Enhancer</div>
